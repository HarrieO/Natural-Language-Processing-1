...
for n,c_id in enumerate(c_id_list):
	...
	plt.plot(1.0/261394 * c_table['features'],c_table['test_accuracy'],label=c_names_list[n],color=color_list[n],linestyle='_')

	c_table = d_table[d_table['classifier_id']==c_id]
	plt.plot(1.0/9477 * c_table['features'],c_table['test_accuracy'],label=c_names_list[n],color='m',linestyle='-')
